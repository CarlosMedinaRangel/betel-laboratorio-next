...
// Definimos el tipo igual que en tu Base de Datos
type Product = {
  _id: string; // Mongo usa _id, no id
  name: string;
  unit: number;
  stock: number;
  price: number;
...
export default function ProductsTable() {
  ...
  // Esta función busca los datos en tu API
  const fetchProducts = async () => {
    try {
      const res = await fetch("/api/product"); // Tu ruta API
            ...
            {products.length === 0 ? (
              ...
              products.map((product) => (
                <tr key={product._id} className="hover:bg-primary/5 transition-colors group">
                  ...
                  {/* Nombre + Categoría (Icono simple por ahora) */}
